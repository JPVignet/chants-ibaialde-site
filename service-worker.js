console.log("SERVICE WORKER V22 ACTIF");

const CACHE_NAME = "chants-cache-v23";
...
self.addEventListener("fetch", event => {
  ...
  // 🔥 1. HTML = toujours réseau d'abord
  ...
  // 🔥 2. autres fichiers = cache first
  event.respondWith(
    caches.match(event.request).then(cached => {
      return cached || fetch(event.request).then(response => {
        const copy = response.clone();
        caches.open("chants-cache-v8").then(cache => {
