const CORE_CACHE = "chants-core-v1";
const CHANTS_CACHE = "chants-data-v1";
...
// ⚡ INSTALL rapide (UI immédiate)
self.addEventListener("install", event => {
  self.skipWaiting();

  event.waitUntil(
    caches.open(CORE_CACHE).then(cache => cache.addAll(coreFiles))
  );
});
...
self.addEventListener("activate", event => {
  event.waitUntil(
    (async () => {
      ...
      await Promise.all(
        keys.map(k => {
          if (![CORE_CACHE, CHANTS_CACHE].includes(k)) {
    ...
    caches.match(event.request).then(cached => {
      return (
        ...
        fetch(event.request).then(res => {
          ...
          // on stocke les chants dans cache secondaire
          caches.open(CHANTS_CACHE).then(cache => {
...
// 🚀 PRECHARGEMENT progressif (non bloquant)
async function preloadChants() {
  const urls = await getChantsList();

  const cache = await caches.open(CHANTS_CACHE);
